...
- **存储区为 `sync`**：options.tsx 经 `@plasmohq/storage`（默认区 `sync`）写入；content.tsx 必须从 `chrome.storage.sync` 读取、`onChanged` 监听 `area === "sync"`。若 content 误读 `local`，会读到 `undefined` 而静默回退 `bilingual`，导致 `replace` 不生效（v1.2.1 修复的缺陷）。
- 内容脚本：
...
## 15. v1.2.1 变更摘要（相对 v1.2）
1. 修复 `replace` 模式不生效：`content.tsx` 原从 `chrome.storage.local` 读 `displayMode`、监听 `area === "local"`，而 `options.tsx` 经 `@plasmohq/storage`（默认区 `sync`）写入，区域不一致导致 content 永远读到 `undefined`、静默回退 `bilingual`。改为读 `chrome.storage.sync`、监听 `area === "sync"`（§4）。